...
import { Input, Dropdown, Alert, ModalAddNewHero, ModalUpdateHero, ModalAddNewOurProduct, ModalUpdateOurProduct, ModalUpdateUpcomingProject, PopupImagePreview } from '../../../components';
...
const ListUpcomingProject = () => {
	...
	const [ListUpcomingProject, setListUpcomingProject] = useState([])
	...
	useEffect(() => {
        ...
        if (CookieParamKey == null || CookieParamKey === "" || CookieUsername == null || CookieUsername === "") {
            ...
            dispatch(setForm("SubPageActive","UPCOMING-PROJECT"))

            getListUpcomingProject(1, "", "", "")
        }
    ...
    // { ---------- start curl ----------
    const getListUpcomingProject = (page, position, searchValue, searchType, id) => {
		...
		var url = paths.URL_API_ADMIN + 'UpcomingProject';
		...
		.then((data) => {
			...
			if (data.ErrorCode === "0") {
                if (position == "detail") {
                    ...
                    setListUpcomingProject(data.Result)
    ...
    const handleUpdate = (status, paramUpdate, id) => {
        ...
        var idUpcomingProject = ""
        var errorMessage = ""
        if (paramUpdate == "status") {
            idUpcomingProject = id
    ...
    const handleChangeStatus = (id, status) => {
        var vListUpcomingProject = ListUpcomingProject
        ...
        vListUpcomingProject.map((item,index) => {
            if (item.Id === id) {
                vListUpcomingProject[index].Status = vStatus
                handleUpdate(vStatus, "status", id)
            }
        })
        setListUpcomingProject(vListUpcomingProject)
    }

    const handlePageClick = (data) => {
        let currentPage = data.selected + 1
        setPaging(data.selected)
        setListUpcomingProject([])
        getListUpcomingProject(currentPage, "", "", "", "")
    }
                        ...
                        <div>
                            ...
                            <Table striped bordered hover responsive cellspacing="0" cellpadding="10" style={{ fontSize:13, borderColor:'white', width:'100%' }}>
                                ...
                                <Tbody>
                                    ...
                                    :
                                    ListUpcomingProject.length > 0 ? ListUpcomingProject.map((item,index)=>{
                                    return <Tr style={{
                                        marginBottom:20,
                                        borderBottomWidth:1,
                                            ...
                                            <td>
                                                <img 
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        getListUpcomingProject(1, "detail", "", "", item.Id)
                                                    }}
                                                />
                                            </td>
                                        </Tr>
...
export default ListUpcomingProject